...
class JamFilesIRTests {
  public:
	static void registerAllTests(TestFramework &framework) {
		...
		framework.addTest("IR - test_println_complex.jam",
		                  testPrintlnComplexJam);
		framework.addTest("IR - test_comprehensive_if.jam",
		                  testComprehensiveIfJam);
		framework.addTest("IR - test_string_slice_demo.jam",
		                  testStringSliceDemoJam);
		framework.addTest("IR - test_bool_comprehensive.jam",
		                  testBoolComprehensiveJam);
	}
  ...
  private:
	...
	static std::string compileJamFile(const std::string &filename) {
		...
		std::string command = "cd " + projectRoot + " && ./jam.out --emit-ir " +
		                      filename + " 2>&1";
	...
	static void testFinalDemoJam() {
		...
		ASSERT_CONTAINS(
		    ir, "define i1 @validate_user(i8 %age, i1 %isActive, i16 %score)");
		ASSERT_CONTAINS(
		    ir, "define i8 @calculate_status(i1 %hasPermission, i8 %level)");
	...
	static void testBoolComprehensiveJam() {
		...
		ASSERT_CONTAINS(
		    ir, "define i1 @test_bool_function_calls(i1 %enabled, i8 %count)");
