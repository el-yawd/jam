...
class LoopTests {
  ...
  private:
	...
	static std::string compileAndRun(const std::string &jamCode) {
		...
		std::string compileCmd = "cd " + projectRoot + " && ./jam.out " +
		                         tempFile + " > /dev/null 2>&1";
...
void LoopTests::registerAllTests(TestFramework &framework) {
	...
	framework.addTest("Loop - For loop with break and continue",
	                  testForLoopWithBreakAndContinue);
	framework.addTest("Loop - While loop with break", testWhileLoopWithBreak);
	framework.addTest("Loop - While loop with false condition",
	                  testWhileLoopWithFalseCondition);
	framework.addTest("Loop - Nested for loops", testNestedForLoops);
	framework.addTest("Loop - For loop with different types",
	                  testForLoopWithDifferentTypes);
	framework.addTest("Loop - Empty for loop", testEmptyForLoop);
	framework.addTest("Loop - For loop variable access",
	                  testForLoopVariableAccess);
}