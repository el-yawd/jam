...
class ParserTests {
  public:
	static void registerTests(TestFramework &framework) {
		...
		framework.addTest("Parser - Function with Parameters",
		                  testFunctionWithParameters);
		framework.addTest("Parser - Variable Declaration",
  ...
  private:
	...
	static void testVariableDeclaration() {
		auto functions =
		    parseCode("fn test() u8 { var x: u8 = 42; return x; }");
	...
	static void testConstDeclaration() {
		auto functions =
		    parseCode("fn test() u8 { const x: u8 = 42; return x; }");
	...
	static void testBinaryExpression() {
		auto functions = parseCode(
		    "fn test() u8 { const result: u8 = 10 + 20; return result; }");
	...
	static void testTypeAnnotations() {
		auto functions =
		    parseCode("fn test() u32 { const a: u8 = 255; const b: u16 = "
		              "65535; const c: u32 = 4294967295; return c; }");

		ASSERT_EQ(1, functions.size());
		ASSERT_EQ("u32", functions[0]->ReturnType);
		ASSERT_EQ(4,
		          functions[0]->Body.size());  // 3 const declarations + return
	}
};