...
class LexerTests {
  ...
  private:
	...
	static void testOperators() {
		...
		ASSERT_EQ(TOK_OPEN_PAREN, tokens[3].type);
		ASSERT_EQ(TOK_CLOSE_PAREN, tokens[4].type);
		ASSERT_EQ(TOK_OPEN_BRACE, tokens[5].type);
		ASSERT_EQ(TOK_CLOSE_BRACE, tokens[6].type);
		ASSERT_EQ(TOK_COMMA, tokens[7].type);
		ASSERT_EQ(TOK_SEMI, tokens[8].type);
	}

	static void testComplexExpression() {
		Lexer lexer("fn test(a: u8, b: u16) u32 { const result: u32 = a + b; "
		            "return result; }");
		auto tokens = lexer.scanTokens();
