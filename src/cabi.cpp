...
llvm::CallingConv::ID CAbi::getCallingConvention() const {
	...
	case OS::Windows:
		...
		if (target_.abi == ABI::MSVC) { return llvm::CallingConv::Win64; }
...
llvm::Type *CAbi::toCAbiType(llvm::Type *jamType,
...
void FunctionAbi::applyToFunction(llvm::Function *func,
                                  const CAbi &cabi) const {
	// Set linkage
	if (isExtern) {
	...
	// Set attributes
	if (isInline) { func->addFnAttr(llvm::Attribute::AlwaysInline); }

	if (noReturn) { func->addFnAttr(llvm::Attribute::NoReturn); }
}
